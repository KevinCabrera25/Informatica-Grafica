...
float _left = 0.0, _right = 1.0f / 7.0f, _top = 1.0, _bottom = 3.0f / 4.0f;// run
...
void MoveRun() 
{
	...
	float xAmount = 1.0f / 7.0f;
	float yAmount = 1.0f / 4.0f;

	// Cada medio segundo cambia de frame
	if (frames >= 0.01f)
	{
		// Si se llega a 1 llega al final
		if (_right >= 1.0f)
		{
			_right = xAmount;
			_top -= yAmount;
